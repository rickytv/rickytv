...
mysettings = xbmcaddon.Addon(id = 'plugin.video.RickyIPTV')
...
def main():
	...
	if len(online_m3u) > 0:	
		add_dir('[COLOR red][B]>> TV <<[/B][/COLOR]', u_tube, 2, icon, fanart)
	if len(local_m3u) > 0:	
		add_dir('[COLOR magenta][B]>> Adult <<[/B][/COLOR]', u_tube, 3, icon, fanart)
	if len(online_xml) > 0:	
